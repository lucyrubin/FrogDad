...
func _ready():
	...
	if MasterScript.currentQuestNum == -1 :
		...
		# show the baby jar being carried
		...
		PopUpNode.show_dialog_box([
			{avatar = "", text = "Wow, they've grown up so fast, but now they're hungry all the time."},
...
func _on_ToggleQuestButton_pressed():
	...
	if MasterScript.frog_dad_state == "":
		...
		if MasterScript.currentQuestArray.empty():
			...
			dynamic_font.font_data = load("res://Fonts/defaultfont.ttf")
	...
	for quest in MasterScript.currentQuestArray:
		if quest[0] == "resource collection":
			...
			if MasterScript.num_kids_fed_flies == 3: 
				PopUpNode.visible = true
				FrogDad.set_fly_jar_visiblity(false)
				PopUpNode.show_dialog_box([{avatar = "", text = "Phew! They have been fed."},
					{avatar = "", text = "Oh my goodness, they are going to grow up soon."}, 
					{avatar = "", text = "Time flies."}],
